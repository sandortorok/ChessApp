...
export default function PodiumCard({ player, rank }: PodiumCardProps) {
  ...
  return (
    <div
      className={`${config.bgClass} border-2 ${config.borderClass} rounded-xl p-6 text-center transform ${config.translateClass} ${config.shadowClass}`}
    >
      <div className={`${config.size} mb-3`}>{config.emoji}</div>
      <div className={`${config.nameSize} font-bold text-white mb-1`}>
        {player.name}
      </div>
      <div
        className={`${config.eloSize} font-bold ${getEloColor(player.elo)} mb-2`}
      >
        {player.elo}
      </div>
      ...
      <div
        className={`mt-${rank === 1 ? '4' : '3'} flex justify-center gap-4 ${config.textSize} text-slate-300`}
      >
