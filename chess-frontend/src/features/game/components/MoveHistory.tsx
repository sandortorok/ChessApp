      ...
      <div className="bg-slate-700/50 border-b border-emerald-600/30 p-3 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          {viewingHistoryIndex !== null && onGoToLatest && (
            ...
            >
              <span className="flex items-center gap-1">🔄 Élő játék</span>
            </button>
          )}
        </div>
      </div>
            ...
            <div className="grid grid-cols-4 gap-1.5 mt-3 pt-3 border-t border-emerald-600/30 flex-shrink-0">
              ...
              <button
                onClick={() =>
                  ...
                  onViewMove(Math.max(0, viewingHistoryIndex - 1))
                }
                disabled={
                  viewingHistoryIndex === null || viewingHistoryIndex === 0
                }
                className="bg-slate-700/50 hover:bg-slate-700/70 disabled:bg-slate-800/30 disabled:text-slate-600 text-white px-2 py-2 rounded-lg font-bold transition-all border border-emerald-600/20 hover:border-emerald-500/40 disabled:border-emerald-600/10 text-sm"
                title="Előző lépés"
              >
              ...
              <button
                onClick={() =>
                  viewingHistoryIndex !== null &&
                  onViewMove(
                    Math.min(moveHistory.length - 1, viewingHistoryIndex + 1)
                  )
                }
                disabled={
                  viewingHistoryIndex === null ||
                  viewingHistoryIndex === moveHistory.length - 1
                }
                className="bg-slate-700/50 hover:bg-slate-700/70 disabled:bg-slate-800/30 disabled:text-slate-600 text-white px-2 py-2 rounded-lg font-bold transition-all border border-emerald-600/20 hover:border-emerald-500/40 disabled:border-emerald-600/10 text-sm"
