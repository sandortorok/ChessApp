...
export function PlayerSlot({
  ...
  const getBorderColor = () => {
    ...
    if (playerWon)
      return 'border-green-500/60 bg-green-500/5 ring-2 ring-green-500/20';
    if (playerLost)
      return 'border-red-500/60 bg-red-500/5 ring-2 ring-red-500/20';
    if (isAborted)
      return 'border-orange-500/60 bg-orange-500/5 ring-2 ring-orange-500/20';
    return 'border-amber-500/60 bg-amber-500/5 ring-2 ring-amber-500/20';
  };
  ...
  return (
    ...
    >
      {isCurrentUser && (
        <div
          className={`absolute -top-2 -right-2 text-slate-900 text-xs font-bold px-2 py-0.5 rounded-full shadow-lg ${getBadgeColor()}`}
        >
          {getBadgeLabel()}
        </div>
      )}
          ...
          (isGuest(player) ? (
            ...
            <span className="text-emerald-400 font-semibold text-sm bg-emerald-500/20 px-2 py-0.5 rounded flex items-center gap-1.5">
              ...
              {gameStatus === 'ended' &&
                ...
                eloChange !== null && (
                  <span
                    className={`text-xs font-bold ${eloChange > 0 ? 'text-green-400' : 'text-red-400'}`}
                  >
                    ({eloChange > 0 ? '+' : ''}
                    {eloChange})
                  </span>
